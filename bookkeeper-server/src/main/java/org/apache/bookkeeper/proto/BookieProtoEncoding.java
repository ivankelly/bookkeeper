...
public class BookieProtoEncoding {
    ...
    public static class ResponseDecoder extends OneToOneDecoder {
        ...
        public Object decode(ChannelHandlerContext ctx, Channel channel, Object msg)
            ...
            final ChannelBuffer buffer2 = buffer.slice();
            if (version >= BookieProtocol.FIRST_PROTOBUF_PROTOCOL_VERSION) {
                try {
                    builder.mergeDelimitedFrom(new ChannelBufferInputStream(buffer2));
                    DataFormats.ResponseHeader h = builder.build();
                    if (h.hasReadResponse()) {
                        return new BookieProtocol.Response(version, h, buffer2.slice());
                    } else {
                        return new BookieProtocol.Response(version, h);
                    }
                } catch (InvalidProtocolBufferException e) {
                    LOG.warn("Couldn't decode with protobuf."
                             + "This could be because an old server returned EBadVersion,"
                             + " but used the wrong protocol version. "
                             + " Trying to decode the legacy protocol.");
                }
            }

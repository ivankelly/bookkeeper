...
public class PerChannelBookieClient extends SimpleChannelHandler implements ChannelPipelineFactory {
    ...
    public PerChannelBookieClient(ClientConfiguration conf, OrderedSafeExecutor executor,
        ...
        this.statsLogger = parentStatsLogger.scope(addr.getHostName().replace('.', '_').replace('-', '_')+"_"+addr.getPort());
        readEntryOpLogger = statsLogger.getOpStatsLogger("read-entry");
        addEntryOpLogger = statsLogger.getOpStatsLogger("add-entry");
        readTimeoutOpLogger = statsLogger.getOpStatsLogger("read-timeout");
        addTimeoutOpLogger = statsLogger.getOpStatsLogger("add-timeout");
    ...
    void addEntry(final long ledgerId, byte[] masterKey, final long entryId, ChannelBuffer toSend, WriteCallback cb,
        ...
        addCompletions.put(completionKey, new AddCompletion(statsLogger, cb, ctx));
    ...
    public void readEntryAndFenceLedger(final long ledgerId, byte[] masterKey,
        ...
        readCompletions.put(key, new ReadCompletion(statsLogger, cb, ctx));
    ...
    static class ReadCompletion extends CompletionValue {
        ...
        public ReadCompletion(final PCBookieClientStatsLogger statsLogger,
                              final ReadEntryCallback originalCallback,
                              final Object originalCtx) {
            super(originalCtx);
            final long requestTimeMillis = MathUtils.now();
            this.cb = null == statsLogger ? originalCallback : new ReadEntryCallback() {
                @Override
    ...
    static class AddCompletion extends CompletionValue {
        ...
        public AddCompletion(final PCBookieClientStatsLogger statsLogger,
                             final WriteCallback originalCallback,
                             final Object originalCtx) {
            super(originalCtx);
            final long requestTimeMillis = MathUtils.now();
            this.cb = null == statsLogger ? originalCallback : new WriteCallback() {
                @Override

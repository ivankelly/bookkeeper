...
public class PerChannelBookieClient extends SimpleChannelHandler
    implements ChannelPipelineFactory, ClientHandshakeHandler.ClientHandshakeCallback {
    ...
    private byte protocolVersion = BookieProtocol.CURRENT_PROTOCOL_VERSION;
    ...
    void addEntry(final DataFormats.AddRequest addRequest, ChannelBuffer toSend,
        ...
        BookieProtocol.Request r = new BookieProtocol.Request(protocolVersion, builder.build(), toSend);
                ...
                public void operationComplete(ChannelFuture future) throws Exception {
                    if (future.isSuccess()) {
                        LOG.debug("Successfully wrote request for adding entry: {}-{}  bookie: {} entry length: {}",
                                new Object[] { addRequest.getLedgerId(), addRequest.getEntryId(),
                                               channel.getRemoteAddress(), entrySize });
                    } else {
                ...
                public void operationComplete(ChannelFuture future) throws Exception {
                    if (future.isSuccess()) {
                        if (LOG.isDebugEnabled()) {
                            LOG.debug("Successfully wrote request to read {}-{} to {}",
                                    new Object[] { readRequest.getLedgerId(), readRequest.getEntryId(),
    ...
    public ChannelPipeline getPipeline() throws Exception {
        ...
        pipeline.addLast("handshakeHandler", new ClientHandshakeHandler(conf, this));
    ...
    void handleAddResponse(BookieProtocol.Response a) {
        if (LOG.isDebugEnabled()) {
            LOG.debug("Got response for add request from bookie: {} for : {}-{}",
                      new Object[] { addr, a.getHeader().getAddResponse().getLedgerId(),
    ...
    void handleReadResponse(BookieProtocol.Response r) {
        if (LOG.isDebugEnabled()) {
            LOG.debug("Got response for read request {}-{} entry length: {}",
                      new Object[] { r.getHeader().getReadResponse().getLedgerId(),
                                     r.getHeader().getReadResponse().getEntryId(),
                                     r.hasData() ? r.getData().readableBytes() : 0 });
        }

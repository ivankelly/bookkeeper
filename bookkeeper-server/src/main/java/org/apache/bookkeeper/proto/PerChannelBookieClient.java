...
import org.apache.bookkeeper.client.BookKeeperClientStats.ClientChannelOp;
...
public class PerChannelBookieClient extends SimpleChannelHandler implements ChannelPipelineFactory {
    ...
    public PerChannelBookieClient(ClientConfiguration conf, OrderedSafeExecutor executor,
        ...
        this.statsLogger = parentStatsLogger.scope(
                "channel-" + addr.getHostName().replace('.', '_').replace('-', '_')
                +"_"+addr.getPort());
        readEntryOpLogger = statsLogger.getOpStatsLogger(ClientChannelOp.READ_ENTRY);
        addEntryOpLogger = statsLogger.getOpStatsLogger(ClientChannelOp.ADD_ENTRY);
        readTimeoutOpLogger = statsLogger.getOpStatsLogger(ClientChannelOp.TIMEOUT_READ_ENTRY);
        addTimeoutOpLogger = statsLogger.getOpStatsLogger(ClientChannelOp.TIMEOUT_ADD_ENTRY);

...
public class EntryMemTable {
    ...
    static class EntrySkipList extends ConcurrentSkipListMap<EntryKey, EntryKeyValue> {
        ...
        EntrySkipList(final LogMark cp) {
            super(EntryKey.COMPARATOR);
            this.cp = cp;
        }

        int compareTo(final LogMark cp) {
    ...
    private EntrySkipList newSkipList() {
        synchronized (markLock) {
            return new EntrySkipList(lastAddedMark);
        }
    }
    ...
    */
    ...
    LogMark snapshot() throws IOException {
        return snapshot(LogMark.MAX_VALUE);
    }
     ...
     * @throws IOException
     */
    LogMark snapshot(LogMark oldCp) throws IOException {
        LogMark cp = null;
    ...
    long flush(final SkipListFlusher flusher) throws IOException {
        return flushSnapshot(flusher, LogMark.MAX_VALUE);
    }
     ...
     */
    public long flush(SkipListFlusher flusher, LogMark checkpoint) throws IOException {
     ...
     */
    private long flushSnapshot(final SkipListFlusher flusher, LogMark checkpoint) throws IOException {
        long size = 0;

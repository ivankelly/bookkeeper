...
public class UpdateLedgerOpTest extends BookKeeperClusterTestCase {
    ...
    public void testManyLedgers() throws Exception {
        ...
        ledgers.add(lh1);
        for (int i = 0; i < 99; i++) {
            ledgers.add(createLedgerWithEntries(bk, 10));
        }
        ...
        updateLedgerOp.updateBookieIdInLedgers(curBookieAddr, toBookieAddr, 5, Integer.MIN_VALUE, 0);
    ...
    public void testLimitLessThanTotalLedgers() throws Exception {
        ...
        BookieSocketAddress toBookieAddr = new BookieSocketAddress(toBookieId.getHostname() + ":"
                + curBookieAddr.getPort());
        UpdateLedgerOp updateLedgerOp = new UpdateLedgerOp(bk, bkadmin);
        updateLedgerOp.updateBookieIdInLedgers(curBookieAddr, toBookieAddr, 7, 5, 2);
        int updatedLedgersCount = getUpdatedLedgersCount(bk, ledgers, toBookieAddr);
        Assert.assertEquals("Failed to update the ledger metadata to use bookie host name", 5, updatedLedgersCount);

        // next execution
        updateLedgerOp.updateBookieIdInLedgers(curBookieAddr, toBookieAddr, 2, 9, 2);
        updatedLedgersCount = getUpdatedLedgersCount(bk, ledgers, toBookieAddr);
        Assert.assertEquals("Failed to update the ledger metadata to use bookie host name", 9, updatedLedgersCount);

        // next execution
        updateLedgerOp.updateBookieIdInLedgers(curBookieAddr, toBookieAddr, 1, 10, 1);
        updatedLedgersCount = getUpdatedLedgersCount(bk, ledgers, toBookieAddr);
    ...
    public void testChangeEnsembleAfterRenaming() throws Exception {
        ...
        BookieSocketAddress toBookieAddr = new BookieSocketAddress(toBookieId.getHostname() + ":"
                + curBookieAddr.getPort());
        UpdateLedgerOp updateLedgerOp = new UpdateLedgerOp(bk, bkadmin);
        updateLedgerOp.updateBookieIdInLedgers(curBookieAddr, toBookieAddr, 5, 100, 5);
    ...
    public void testRenameWhenAddEntryInProgress() throws Exception {
        ...
        UpdateLedgerOp updateLedgerOp = new UpdateLedgerOp(bk, bkadmin);
        updateLedgerOp.updateBookieIdInLedgers(curBookieAddr, toBookieAddr, 5, 100, 25);

...
public abstract class JmsSendReceiveTestSupport extends org.apache.activemq.TestSupport implements MessageListener {
    ...
    protected void setUp() throws Exception {
        ...
        messageCountDef = 100;
        {
            int temp = Integer.getInteger("messageCount", -1);

            if (temp > 0) {
                messageCountDef = temp;
            }
        }

        initializeData(messageCountDef);
    }

    protected void initializeData(int messageCount) {
    ...
    protected void initializeData(HedwigConnectionImpl connection, SessionImpl session) {
        int messageCount = 100;
        if (session.getTransacted() || Session.CLIENT_ACKNOWLEDGE == session.getAcknowledgeMode()){
            messageCount = connection.getHedwigClientConfig().getMaximumOutstandingMessages() - 1;
        }

        initializeData(messageCount);
    }
    ...
    public void testSendReceive() throws Exception {
        initializeData((HedwigConnectionImpl) ((SessionImpl)session).getConnection(), (SessionImpl) session);
        Thread.sleep(1000);
        messages.clear();
        LOG.info("cleared messages, now sending the messages");
    ...
    protected void sendMessages() throws Exception {
        ...
        LOG.info("Sent " + data.length + " messages to " + producerDestination);
    }

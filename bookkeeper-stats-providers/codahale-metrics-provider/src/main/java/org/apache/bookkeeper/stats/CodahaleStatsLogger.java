...
public class CodahaleStatsLogger implements StatsLogger {
    ...
    public OpStatsLogger getOpStatsLogger(Enum statName) {
        Timer success = metrics.timer(name(basename, statName.toString()));
        Timer failure = metrics.timer(name(basename, statName.toString()+"-fail"));
        return new CodahaleOpStatsLogger(success, failure);
    }

    @Override
    public Counter getCounter(Enum statName) {
        final com.codahale.metrics.Counter c = metrics.counter(name(basename, statName.toString()));
        return new Counter() {
            @Override
    ...
    public <T extends Number> void registerGauge(final Enum statName, final Gauge<T> gauge) {
        String metricName = name(basename, statName.toString());
